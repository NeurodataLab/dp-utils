...
class DetectionPreprocessor(MIMOProcessor):
    ...
    def process(self, **kwargs):
        ...
        img_box_flipper_inp_full = merge_dicts(img_crop_out, box_crop_out)
        ...
        image_aug_inp = {name: img_box_flipper_out[name] for name in self._image_aug.provide_input}
        image_aug_out = self._image_aug.process(**image_aug_inp)

        batchifier_inp_full = merge_dicts(box_crop_out, img_box_flipper_out)
        ...
        return {k: v for k, v in merge_dicts(image_aug_out, box_crop_out, batchifier_out).items()
                if k in self.provide_output}