...
class MultiProcessPrefetchIterator(BaseIterator):
    def __init__(self, num_processes, max_tasks=100, max_results=1000, *args, **kwargs):
        super(MultiProcessPrefetchIterator, self).__init__(*args, **kwargs)
        ...
        self._fetch_storage = Queue(self._max_results)
        fetch_func = self._make_fetch_func()

        self._fetcher = th.Thread(target=fetch_func, args=(self._output_storage, self._fetch_storage))
        self._fetcher.start()

    @staticmethod
    def _make_fetch_func():
        def fetch_func(result_queue, storage_queue):
            while True:
                idx, result = result_queue.get(True, 10)
                storage_queue.put((idx, result))
        return fetch_func
    ...
    def next(self):
        ...
        print('pre', self._fetch_storage.qsize())

        while sample_num < self._batch_size:
            try:
                idx, data_dict = self._fetch_storage.get(True, 10)
        ...
        print('post', self._fetch_storage.qsize())
